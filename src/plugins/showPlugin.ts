...
export const showPlugin = () => element => {
  ...
  if (list.length === 0) {
    doc.addEventListener("show-" + name, (e:CustomEventInit) => listener(name, e.detail));
  } else {
...
const listener = (name:string, id:string) => {
  const list = hides[name] ? hides[name] : [];
  ...
  const newlist = others.map(b => {
    const display =
      b.element.style.display === "none" ? b.display : b.element.style.display;
